//#region ../node_modules/.pnpm/tweakpane@4.0.5/node_modules/tweakpane/dist/tweakpane.js
...
function deepMerge(r1, r2) {
	return Array.from(new Set([...Object.keys(r1), ...Object.keys(r2)])).reduce((result, key) => {
		const v1 = r1[key];
		const v2 = r2[key];
...
var TpError = class TpError {
	static alreadyDisposed() {
		return new TpError({ type: "alreadydisposed" });
	}
	static notBindable() {
		return new TpError({ type: "notbindable" });
	}
	static notCompatible(bundleId, id) {
		return new TpError({
			type: "notcompatible",
			context: { id: `${bundleId}.${id}` }
		});
	}
...
var BindingTarget = class BindingTarget {
	constructor(obj, key) {
		this.obj_ = obj;
		this.key = key;
	}
...
var ComplexValue = class {
	...
	setRawValue(rawValue, options) {
		...
		if (!!this.equals_(prevValue, constrainedValue) && !opts.forceEmit) return;
...
var ValueMap = class ValueMap {
	constructor(valueMap) {
		this.emitter = new Emitter();
		this.valMap_ = valueMap;
		for (const key in this.valMap_) this.valMap_[key].emitter.on("change", () => {
			this.emitter.emit("change", {
				key,
				sender: this
			});
	...
	static createCore(initialValue) {
		return Object.keys(initialValue).reduce((o, key) => {
			return Object.assign(o, { [key]: createValue(initialValue[key]) });
		}, {});
	}
	static fromObject(initialValue) {
		return new ValueMap(this.createCore(initialValue));
	}
	get(key) {
...
var StepConstraint = class {
	...
	constrain(value) {
		...
		return o + Math.round((value - o) / this.step) * this.step;
	}
};
...
var readNumericLiteral = combineReader([combineReader([
...
function parseEcmaNumberExpression(text) {
	...
	if (!expr) return null;
	if (expr.cursor + readWhitespace(text, expr.cursor).length !== text.length) return null;
...
function getSuitablePointerScale(params, rawValue) {
	...
	return base === 0 ? .1 : Math.pow(10, Math.floor(Math.log10(base)) - 1);
}
...
function parseObject(value, keyToParserMap) {
	return forceCast(Object.keys(keyToParserMap).reduce((tmp, key) => {
		if (tmp === void 0) return;
		const parser = keyToParserMap[key];
		const result = parser(value[key]);
		return result.succeeded ? Object.assign(Object.assign({}, tmp), { [key]: result.value }) : void 0;
	}, {}));
}
function parseArray(value, parseItem) {
	return value.reduce((tmp, item) => {
		...
		if (!result.succeeded || result.value === void 0) return;
...
function getCanvasContext(canvasElement) {
	...
	if (!win) return null;
	return "document" in win ? canvasElement.getContext("2d", { willReadFrequently: true }) : null;
}
...
var LabelView = class {
	constructor(doc, config) {
		...
		bindValueMap(config.props, "label", (value) => {
			if (isEmpty(value)) this.element.classList.add(cn$r(void 0, "nol"));
			else {
...
var LabelController = class {
	...
	importProps(state) {
		return importBladeState(state, null, (p) => ({ label: p.optional.string }), (result) => {
			this.props.set("label", result.label);
			return true;
		});
	}
	exportProps() {
		return exportBladeState(null, { label: this.props.get("label") });
	}
};
...
var BindingController = class extends LabeledValueBladeController {
	...
	importState(state) {
		return importBladeState(state, (_s) => super.importState(excludeValue(state)), (p) => ({ tag: p.optional.string }), (result) => {
			this.tag = result.tag;
			return true;
		});
	}
...
var InputBindingController = class extends BindingController {
	importState(state) {
		return importBladeState(state, (s) => super.importState(s), (p) => ({ binding: p.required.object({ value: p.required.raw }) }), (result) => {
...
function fillBuffer(buffer, bufferSize) {
	while (buffer.length < bufferSize) buffer.push(void 0);
}
...
var MonitorBindingController = class extends BindingController {
	exportState() {
		return exportBladeState(() => super.exportState(), { binding: { readonly: true } });
	}
};
...
var ButtonApi = class extends BladeApi {
	...
	on(eventName, handler) {
		const bh = handler.bind(this);
		this.controller.buttonController.emitter.on(eventName, (ev) => {
			bh(new TpMouseEvent(this, ev.nativeEvent));
		});
		return this;
	}
	off(eventName, handler) {
		this.controller.buttonController.emitter.off(eventName, handler);
		return this;
	}
};
...
var ButtonController = class {
	...
	importProps(state) {
		return importBladeState(state, null, (p) => ({ title: p.optional.string }), (result) => {
			this.props.set("title", result.title);
			return true;
		});
	}
	exportProps() {
		return exportBladeState(null, { title: this.props.get("title") });
	}
	onClick_(ev) {
...
var ButtonBladePlugin = createPlugin({
	...
	controller(args) {
		return new ButtonBladeController(args.document, {
			blade: args.blade,
			buttonProps: ValueMap.fromObject({ title: args.params.title }),
			labelProps: ValueMap.fromObject({ label: args.params.label }),
...
var ContainerBladeController = class extends BladeController {
	...
	importState(state) {
		return importBladeState(state, (s) => super.importState(s), (p) => ({ children: p.required.array(p.required.raw) }), (result) => {
	...
	exportState() {
		return exportBladeState(() => super.exportState(), { children: this.rackController.rack.children.map((c) => c.exportState()) });
	}
};
...
var NestedOrderedSet = class {
	...
	find(callback) {
		for (const item of this.allItems()) if (callback(item)) return item;
		return null;
	}
	includes(item) {
...
function createBlade() {
	return new ValueMap({ positions: createValue([], { equals: deepEqualsArray }) });
}
var Foldable = class Foldable extends ValueMap {
	constructor(valueMap) {
		super(valueMap);
	}
	static create(expanded) {
		...
		return new Foldable(ValueMap.createCore(coreObj));
	}
	get styleExpanded() {
		var _a;
	...
	bindExpandedClass(elem, expandedClassName) {
		const onExpand = () => {
			if (this.styleExpanded) elem.classList.add(expandedClassName);
...
var FolderView = class {
	constructor(doc, config) {
		...
		bindValueMap(config.props, "title", (title) => {
			if (isEmpty(title)) this.element.classList.add(this.className_(void 0, "not"));
			else this.element.classList.remove(this.className_(void 0, "not"));
		});
		config.viewProps.bindDisabled(buttonElem);
		this.element.appendChild(buttonElem);
		this.buttonElement = buttonElem;
...
var FolderBladePlugin = createPlugin({
	...
	controller(args) {
		return new FolderController(args.document, {
			...
			props: ValueMap.fromObject({ title: args.params.title }),
			viewProps: args.viewProps
		});
	},
	api(args) {
...
var ViewProps = class ViewProps extends ValueMap {
	constructor(valueMap) {
		var _a;
		super(valueMap);
	...
	getGlobalDisabled_() {
		...
		return (parent ? parent.globalDisabled.rawValue : false) || this.get("disabled");
	}
...
var TabItemView = class {
	constructor(doc, config) {
		...
		bindValueMap(config.props, "selected", (selected) => {
			if (selected) this.element.classList.add(cn$m(void 0, "sel"));
			else this.element.classList.remove(cn$m(void 0, "sel"));
		});
		const buttonElem = doc.createElement("button");
...
var TabPageController = class extends ContainerBladeController {
	constructor(doc, config) {
		const view = new TabPageView(doc, { viewProps: config.viewProps });
		super(Object.assign(Object.assign({}, config), {
			rackController: new RackController({
				blade: config.blade,
				element: view.containerElement,
...
var TabApi = class extends ContainerBladeApi {
	...
	addPage(params) {
		...
		const pc = new TabPageController(doc, {
			...
			props: ValueMap.fromObject({ selected: false }),
			viewProps: ViewProps.create()
		});
...
var ListConstraint = class {
	constructor(options) {
		this.values = ValueMap.fromObject({ options });
	}
	constrain(value) {
...
function parseListOptions(value) {
	...
	if (Array.isArray(value)) return (_a = parseRecord({ items: value }, (p) => ({ items: p.required.array(p.required.object({
...
var ListController = class {
	...
	importProps(state) {
		return importBladeState(state, null, (p) => ({ options: p.required.custom(parseListOptions) }), (result) => {
			this.props.set("options", normalizeListOptions(result.options));
			return true;
		});
	}
	exportProps() {
		return exportBladeState(null, { options: this.props.get("options") });
	}
};
...
var TextController = class {
	...
	onInputChange_(e) {
		const value = forceCast(e.currentTarget).value;
...
function getStepForKey(keyScale, keys) {
	const step = keyScale * (keys.altKey ? .1 : 1) * (keys.shiftKey ? 10 : 1);
...
var PointerHandler = class {
	constructor(element) {
		...
		this.elem_ = element;
		this.emitter = new Emitter();
		element.addEventListener("touchstart", this.onTouchStart_, { passive: false });
		element.addEventListener("touchmove", this.onTouchMove_, { passive: true });
...
var NumberTextView = class {
	constructor(doc, config) {
		...
		if (config.arrayPosition) this.element.classList.add(cn$h(void 0, config.arrayPosition));
		config.viewProps.bindClassModifiers(this.element);
		const inputElem = doc.createElement("input");
...
var NumberTextController = class {
	...
	onInputKeyUp_(ev) {
		if (getStepForKey(this.props.get("keyScale"), getVerticalStepKeys(ev)) === 0) return;
...
var SliderController = class {
	...
	onKeyUp_(ev) {
		if (getStepForKey(this.props.get("keyScale"), getHorizontalStepKeys(ev)) === 0) return;
		this.value.setRawValue(this.value.rawValue, {
			forceEmit: true,
			last: true
		});
...
var CSS_VAR_MAP = { containerUnitSize: "cnt-usz" };
...
var BooleanInputPlugin = createPlugin({
	...
	controller: (args) => {
		...
		if (lc) return new ListController(doc, {
			props: new ValueMap({ options: lc.values.value("options") }),
			value,
			viewProps: args.viewProps
		});
...
function rgbToHsvInt(r, g, b) {
	...
	const bp = constrainRange(b / 255, 0, 1);
	const cmax = Math.max(rp, gp, bp);
	const d = cmax - Math.min(rp, gp, bp);
	let h;
	if (d === 0) h = 0;
...
function convertColor(components, from, to) {
	...
	return convertColorType(MODE_CONVERTER_MAP[from.mode][to.mode](...intComps), to.mode, "int", to.type);
}
var IntColor = class IntColor {
	static black() {
		return new IntColor([
			0,
			0,
...
function parseCssNumberOrPercentage(text, max) {
	...
	return Math.min(parseFloat(m[1]) * .01 * max, max);
}
...
function detectStringColorFormat(text, type = "int") {
	...
	if (!r) return null;
	if (r.notation === "hex" && type !== "float") return Object.assign(Object.assign({}, r), { type: "int" });
	if (r.notation === "func") return Object.assign(Object.assign({}, r), { type });
	return null;
}
...
function colorToHexRgbString(value, prefix = "#") {
	return `${prefix}${removeAlphaComponent(value.getComponents("rgb")).map(zerofill).join("")}`;
}
...
function colorToFunctionalRgbString(value) {
	...
	return `rgb(${removeAlphaComponent(mapColorType(value, "int").getComponents("rgb")).map((comp) => formatter(comp)).join(", ")})`;
}
function colorToFunctionalRgbaString(value) {
	const aFormatter = createNumberFormatter(2);
	const rgbFormatter = createNumberFormatter(0);
	return `rgba(${mapColorType(value, "int").getComponents("rgb").map((comp, index) => {
		return (index === 3 ? aFormatter : rgbFormatter)(comp);
	}).join(", ")})`;
}
function colorToFunctionalHslString(value) {
	...
	return `hsl(${removeAlphaComponent(mapColorType(value, "int").getComponents("hsl")).map((comp, index) => formatters[index](comp)).join(", ")})`;
}
function colorToFunctionalHslaString(value) {
	...
	return `hsla(${mapColorType(value, "int").getComponents("hsl").map((comp, index) => formatters[index](comp)).join(", ")})`;
}
function colorToObjectRgbString(value, type) {
	...
	return `{${removeAlphaComponent(mapColorType(value, type).getComponents("rgb")).map((comp, index) => `${names[index]}: ${formatter(comp)}`).join(", ")}}`;
}
...
function colorToObjectRgbaString(value, type) {
	...
	return `{${mapColorType(value, type).getComponents("rgb").map((comp, index) => {
...
var APaletteController = class {
	...
	handlePointerEvent_(d, opts) {
		...
		const [h, s, v] = this.value.rawValue.getComponents("hsv");
	...
	onKeyUp_(ev) {
		if (getStepForKey(getKeyScaleForColor(true), getHorizontalStepKeys(ev)) === 0) return;
		this.value.setRawValue(this.value.rawValue, {
			forceEmit: true,
			last: true
		});
...
function createComponentController(doc, config, index) {
	return new NumberTextController(doc, {
		...
		props: ValueMap.fromObject({
			...
			pointerScale: config.colorType === "float" ? .01 : 1
		}),
		value: createValue(0, { constraint: createConstraint$5(config.colorMode, config.colorType, index) }),
		viewProps: config.viewProps
	});
}
	...
	return [
		...
		connectValues({
			...
			backward(p, s) {
				...
				const comps = mapColorType(p, config.colorType).getComponents(pickedMode);
				comps[i] = s;
				return mapColorType(createColor(appendAlphaComponent(removeAlphaComponent(comps), comps[3]), pickedMode, config.colorType), "int");
			}
		});
		return c;
	});
...
function createHexController(doc, config) {
	const c = new TextController(doc, {
		...
		props: ValueMap.fromObject({ formatter: colorToHexRgbString }),
...
var HPaletteController = class {
	...
	handlePointerEvent_(d, opts) {
		...
		const [, s, v, a] = this.value.rawValue.getComponents("hsv");
		this.value.setRawValue(new IntColor([
			hue,
			s,
			v,
	...
	onKeyUp_(ev) {
		if (getStepForKey(getKeyScaleForColor(false), getHorizontalStepKeys(ev)) === 0) return;
		this.value.setRawValue(this.value.rawValue, {
			forceEmit: true,
			last: true
		});
...
var SvPaletteView = class {
	...
	update_() {
		...
		if (!ctx) return;
		const hsvComps = this.value.rawValue.getComponents("hsv");
	...
	constructor(doc, config) {
		...
		this.alphaIcs_ = config.supportsAlpha ? {
			...
			text: new NumberTextController(doc, {
				...
				value: createValue(0, { constraint: new DefiniteRangeConstraint({
					min: 0,
					max: 1
				}) }),
				viewProps: this.viewProps
...
var ColorController = class {
	constructor(doc, config) {
		...
		this.textC_ = new TextController(doc, {
			parser: config.parser,
			props: ValueMap.fromObject({ formatter: config.formatter }),
			value: this.value,
			viewProps: this.viewProps
		});
		...
		this.popC_ = config.pickerLayout === "popup" ? new PopupController(doc, { viewProps: this.viewProps }) : null;
...
function equalsColor(v1, v2) {
	...
	for (let i = 0; i < comps1.length; i++) if (comps1[i] !== comps2[i]) return false;
	return true;
}
...
function writeRgbaColorObject(target, value, type) {
	const obj = mapColorType(value, type).toRgbaObject();
...
function colorFromObject(value, type) {
	...
	if (type === "int") return new IntColor(createColorComponentsFromRgbObject(value), "rgb");
	if (type === "float") return new FloatColor(createColorComponentsFromRgbObject(value), "rgb");
...
function createAxisController(doc, config, index) {
	return new NumberTextController(doc, {
		...
		value: createValue(0, { constraint: config.axes[index].constraint }),
		viewProps: config.viewProps
	});
}
var PointNdTextController = class {
	constructor(doc, config) {
		...
		this.view = new PointNdTextView(doc, { textViews: this.acs_.map((ac) => ac.view) });
	}
	get textControllers() {
		return this.acs_;
	}
...
var Point2dPickerView = class {
	constructor(doc, config) {
		...
		if (config.layout === "popup") this.element.classList.add(cn$4(void 0, "p"));
		config.viewProps.bindClassModifiers(this.element);
...
var Point2dInputPlugin = createPlugin({
	...
	accept: (value, params) => {
		...
		const result = parseRecord(params, (p) => Object.assign(Object.assign({}, createPointDimensionParser(p)), {
			...
			y: p.optional.object(Object.assign(Object.assign({}, createPointDimensionParser(p)), { inverted: p.optional.boolean }))
		}));
		return result ? {
			initialValue: value,
			params: result
...
var StringInputPlugin = createPlugin({
	...
	controller: (args) => {
		...
		return new TextController(doc, {
			parser: (v) => v,
			props: ValueMap.fromObject({ formatter: formatString }),
			value,
			viewProps: args.viewProps
		});
	},
...
var MultiLogView = class {
	...
	update_() {
		...
		this.value.rawValue.forEach((value) => {
			if (value !== void 0) lines.push(this.formatter_(value));
		});
...
var BooleanMonitorPlugin = createPlugin({
	...
	binding: { reader: (_args) => boolFromUnknown },
	controller: (args) => {
		var _a;
...
var StringMonitorPlugin = createPlugin({
	...
	binding: { reader: (_args) => stringFromUnknown },
	controller: (args) => {
		var _a;
		const value = args.value;
		if (value.rawValue.length > 1 || args.params.multiline) return new MultiLogController(args.document, {
...
function createInputBindingController(plugin, args) {
	...
	return new InputBindingController(args.document, {
		blade: createBlade(),
		props: ValueMap.fromObject({ label: "label" in args.params ? (_a = params === null || params === void 0 ? void 0 : params.label) !== null && _a !== void 0 ? _a : null : args.target.key }),
...
function createMonitorBindingController(plugin, args) {
	...
	return new MonitorBindingController(args.document, {
		blade: createBlade(),
		props: ValueMap.fromObject({ label: "label" in args.params ? (_c = params === null || params === void 0 ? void 0 : params.label) !== null && _c !== void 0 ? _c : null : args.target.key }),
		value,
		valueController: controller
	});
}
var PluginPool = class {
	...
	createInputBindingApi_(bc) {
		const api = this.pluginsMap_.inputs.reduce((result, plugin) => {
			var _a, _b;
			if (result) return result;
			return (_b = (_a = plugin.api) === null || _a === void 0 ? void 0 : _a.call(plugin, { controller: bc })) !== null && _b !== void 0 ? _b : null;
		}, null);
...
var ListBladePlugin = (function() {
	return {
		...
		controller(args) {
			...
			const value = createValue(args.params.value, { constraint: lc });
			...
			return new LabeledValueBladeController(args.document, {
				blade: args.blade,
				props: ValueMap.fromObject({ label: args.params.label }),
				value,
				valueController: ic
			});
		},
...
var SeparatorController = class extends BladeController {
	...
	*/
	constructor(doc, config) {
		super(Object.assign(Object.assign({}, config), { view: new SeparatorView(doc, { viewProps: config.viewProps }) }));
	}
};
var SeparatorBladePlugin = {
	...
	core: VERSION$1,
	accept(params) {
		const result = parseRecord(params, (p) => ({ view: p.required.constant("separator") }));
		return result ? { params: result } : null;
	},
	controller(args) {
...
var SliderBladePlugin = {
	...
	controller(args) {
		...
		const v = createValue(initialValue, { constraint: drc });
	...
	return {
		...
		controller(args) {
			...
			const ic = new TextController(args.document, {
				...
				props: ValueMap.fromObject({ formatter: (_a = args.params.format) !== null && _a !== void 0 ? _a : ((v) => String(v)) }),
				value: v,
				viewProps: args.viewProps
			});
			return new LabeledValueBladeController(args.document, {
...
var Pane = class extends RootApi {
	constructor(opt_config) {
		...
		const rootController = new RootController(doc, {
			expanded: config.expanded,
			blade: createBlade(),
			props: ValueMap.fromObject({ title: config.title }),
			viewProps: ViewProps.create()
		});
	...
	registerPlugin(bundle) {
		...
		("plugin" in bundle ? [bundle.plugin] : "plugins" in bundle ? bundle.plugins : []).forEach((p) => {
