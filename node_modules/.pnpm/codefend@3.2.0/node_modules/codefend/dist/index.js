...
var PROJECT_KEBAB_CASE_NAME = "codefend";
var PROJECT_DISPLAY_NAME = "Codefend";
var OPTIONS_FILE_NAME = ".codefendrc.json";
var OPTIONS_FILE_PATH = `./${OPTIONS_FILE_NAME}`;
var VERSION = version;
var RC_VERSION = "0.0.1";
var DEFAULT_PROJECT_NAME = "project";
var DEFAULT_PREFIX = "Ox";
var VALID_VAR_REGEX = /^[a-zA-Z_$][a-zA-Z$0-9]*$/;
var MIN_POOL_ITEM_LENGTH = 3;
var HELP_COMMAND_INTRO = `Usage: ${PROJECT_KEBAB_CASE_NAME} [options]

Protect Any Source Code.

Options:`;
var DEFAULT_CLI_OPTION = "-h";
var CLI_OPTIONS = [
  {
    short: "-i",
    long: "--init",
...
var DEFAULT_PARSER_NAME = PARSER_NAMES.default;
var CUSTOM_PARSER_NAME = "custom";
var CODEFEND_CHECK_WARNING = {
  version: {
...
var CHECK_MAX_TABLE_COLUMN_WIDTH = 100;

// src/core/options/options.ts
...
function buildTransformationPoolOption(pool) {
  if (!pool) {
    return [];
  }
  ...
  return Array.from(new Set(parsedPoolWords));
}
function buildParserOptions(parser) {
  const ret = {};
...
function buildGenerationIgnoreOptions(options) {
...
function buildDebugOptions(debugOptions) {
  return {
    stats: typeof debugOptions?.stats === "boolean" ? debugOptions.stats : true,
    ignoredWarnings: debugOptions?.ignoredWarnings === void 0 ? [] : debugOptions?.ignoredWarnings
  };
}
...
// src/core/generation/read.ts
import fs from "fs";
import path from "path";
function readFile(path3, flag = "r") {
  try {
    return fs.readFileSync(path3, { encoding: "utf8", flag });
  } catch (error) {
    console.error(error);
    return null;
  }
...
function initializeCheckResults() {
  return {
    errors: [],
    warnings: []
  };
...
function validateGenerationPackageLock(checkResults, options) {
...
function validateDeprecatedParser(checkResults, options) {
...
function validateTransformationPrefix(checkResults, options) {
...
function validateTransformationPool(checkResults, options) {
  if (!options.transformation?.pool) {
    return;
  }
  const poolArray = typeof options.transformation.pool === "string" ? options.transformation.pool.split(" ") : options.transformation.pool;
  const poolSet = new Set(poolArray);
...
function printCheckResults(checkResults, options) {
  ...
  const warningRows = !hasIgnoredAllWarnings ? checkResults.warnings.map((warning) => [
...
function printIgnoreWarningSentence(warningCode) {
...
// src/core/generation/write.ts
import fs2 from "fs";
function writeFile(path3, data) {
  fs2.writeFileSync(path3, data);
}
...
// src/core/debug/stats.ts
import Table2 from "cli-table3";
function stats(options, runtimeOptions) {
  if (!options.stats) {
    return;
  }
...
function getObfuscatedWordsCount(runtimeOptions) {
...
// src/core/generation/copy.ts
import fs3 from "fs";
import path2 from "path";

// src/core/mapper/mapper.ts
function buildMap(options, runtimeOptions) {
...
function sortMap(runtimeOptions) {
  runtimeOptions.map = Object.keys(runtimeOptions.map).sort((a, b) => {
...
// src/core/replacer/replacer.ts
function replace(options, runtimeOptions) {
...
// src/core/transformation/default.ts
function mapDefaultWords(options, runtimeOptions) {
  let sequence = 0;
  let word;
  for (const key in runtimeOptions.processed.map) {
    if (runtimeOptions.processed.map[key].type == 0 /* default */) {
...
function generateWord(sequence, options) {
  return `${options.prefix}${sequence}`;
}

// src/core/transformation/ignore.ts
function mapIgnoredWords(options, runtimeOptions) {
  options.ignore.forEach((word) => {
    runtimeOptions.map[word] = word;
    if (word in runtimeOptions.processed.map) {
      runtimeOptions.processed.map[word].type = 1 /* ignore */;
    } else {
...
// src/core/transformation/pool.ts
function mapPoolWords(options, runtimeOptions) {
  let sequence = 0;
  let word;
  for (const key in runtimeOptions.processed.map) {
    if (runtimeOptions.processed.map[key].type == 0 /* default */) {
      word = generateWord2(sequence, options);
      if (!word) continue;
      runtimeOptions.map[key] = word;
      runtimeOptions.processed.map[key].target = word;
      runtimeOptions.processed.map[key].type = 3 /* pool */;
      sequence++;
    }
  }
}
function generateWord2(sequence, options) {
...
// src/core/transformation/static.ts
function mapStaticWords(options, runtimeOptions) {
...
// src/core/process/obfuscate.ts
function obfuscate(code, transformationOptions, parserOptions, runtimeOptions) {
  const words = parse({ code, parserOptions });
  buildMap({ words }, runtimeOptions);
  ...
  return replace({ code }, runtimeOptions);
}

// src/core/generation/copy.ts
function copyFolder(from, to, ignoredFilesInGeneration, transformationOptions, parserOptions, runtimeOptions) {
  if (!fs3.existsSync(to)) fs3.mkdirSync(to);
  fs3.readdirSync(from).forEach((element) => {
    if (ignoredFilesInGeneration.includes(element)) return;
    if (fs3.lstatSync(path2.join(from, element)).isFile()) {
      fs3.copyFileSync(
        path2.join(from, element),
        path2.join(
...
// src/core/generation/remove.ts
import fs4 from "fs";
function removeFolder(path3) {
  fs4.rmSync(path3, { recursive: true, force: true });
}

// src/cli/commands/obfuscate.ts
function obfuscateCommand(options) {
  if (!options) {
...
// src/cli/commands/unknown.ts
function unknownCommand(option) {
...
function startCLI() {
  ...
  const selectedArg = inputArgs.length > 0 ? inputArgs[0] ?? DEFAULT_CLI_OPTION : DEFAULT_CLI_OPTION;
